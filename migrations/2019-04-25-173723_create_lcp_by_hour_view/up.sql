CREATE MATERIALIZED VIEW lcp_by_hour_view as
...
WHERE to_timestamp(floor((extract('epoch' from checks.closed_at) / 3600 )) * 3600) AT TIME ZONE 'UTC' >= labor.clock_in
AND   to_timestamp(ceiling((extract('epoch' from checks.closed_at) / 3600 )) * 3600) AT TIME ZONE 'UTC' <= labor.clock_out
GROUP BY start_time, end_time, checks.business_id;

CREATE UNIQUE INDEX lcb_by_hour_and_business
ON lcp_by_hour_view (business_id, start_time, end_time);
